...
loadGeneSet <- function(organism = "hsapiens", enrichDatabase = NULL, enrichDatabaseFile = NULL, enrichDatabaseType = NULL, enrichDatabaseDescriptionFile = NULL,
    ...
    if (organism != "others") { # supported organism
        ...
        for (enrichDb in enrichDatabase) {
            if (is.null(enrichDb) || enrichDb == "others") {
                next
        ...
        for (i in seq_along(enrichDatabaseFile)) {
            ...
            thisGeneSet <- thisGeneSet$mapped %>%
                select(.data$geneSet, .data$description, gene = standardId) %>%
                distinct()
    ...
    if (is.null(geneSet)) {
        stop(enrichDatabaseError(type = "empty"))
    }
    # unlist if just one database, for backward compatibility
    if (length(geneSetDag) == 1) {
        geneSetDag <- geneSetDag[[1]]
    }
    if (length(geneSetNet) == 1) {
        geneSetNet <- geneSetNet[[1]]
    }
