...
plotGeneTable <- function(outputHtmlFile,organism,enrichedSig_sub,enrichMethod,interestingGeneMap,geneSetDes,geneSetNet,enrichDatabase){
	...
	for(i in c(1:nrow(enrichedSig_sub))){
		...
		cat('<tbody>\n',file=outputHtmlFile,append=TRUE,sep="")
		if(gNa==0){
			####GSEA method may have significant categories without leading edge genes. This may also happen when users use 'TOP' method to select the enriched categories. If all categories do not annotate to any interesting genes, all selcted 'TOP' categories should only have 'NA' genes
			for(j in c(1:nrow(g))){
				cat('<tr>\n',file=outputHtmlFile,append=TRUE)
				a <- g[j,]
