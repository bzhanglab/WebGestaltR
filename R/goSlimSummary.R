...
.processData <- function(organism, hostName, geneList, ontology) {
    ...
    if (startsWith(hostName, "file://")) {
        ...
        response <- POST(goUrl, body = list(organism = organism, ontology = ontology, entrezgenes = geneList, version = WEBGESTALT_DATA_VERSION), encode = "json")
        if (response$status_code != 200) {
            stop(webRequestError(response))
        }
        goSlimData <- content(response)[["goslim"]]
        goSlimData <- data.frame(matrix(unlist(lapply(goSlimData, FUN = function(x) {
            x[names]
        })), nrow = length(goSlimData), byrow = TRUE), stringsAsFactors = FALSE)
