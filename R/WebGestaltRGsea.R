WebGestaltRGsea <- function(organism="hsapiens", enrichDatabase="geneontology_Biological_Process", enrichDatabaseFile=NULL, enrichDatabaseType=NULL, enrichDatabaseDescriptionFile=NULL,  interestGeneFile=NULL, interestGene=NULL, interestGeneType=NULL, collapseMethod="mean", minNum=10, maxNum=500, fdrMethod="BH", sigMethod="fdr", fdrThr=0.05, topThr=10, dNum=20, perNum=1000, isOutput=TRUE, outputDirectory=getwd(), projectName=NULL, dagColor="binary", hostName="http://www.webgestalt.org/") {
	...
	gseaRes <- gseaEnrichment(hostName, outputDirectory, projectName, interestGeneList,
		...
		topThr=topThr, perNum=perNum, isOutput=isOutput
	)
	...
	if(!is.null(enrichedSig)){
		...
		if(isOutput==TRUE){
			...
			signedLogFdr <- -log(fdr) * sign(enrichedSig$ES)
			apRes <- affinityPropagation(idsInSet, signedLogFdr)
			wscRes <- weightedSetCover(idsInSet, 1 / signedLogFdr, 10)
