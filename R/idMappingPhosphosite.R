...
idMappingPhosphosite <- function(
	...
	inputGene <- idMappingInput(
		dataType = dataType,
		inputGeneFile = inputGeneFile,
		inputGene = inputGene
	)
	...
	if (startsWith(hostName, "file://")) {
		sourceMap <- read_tsv(
			removeFileProtocol(file.path(
				hostName,
				"xref",
				paste(organism, sourceIdType, "phosphositeSeq.table", sep = "_")
			)),
			col_names = c("phosphositeSeq", "userId"),
			col_types = "cc",
			quote = ""
		) %>%
			...
			targetMap <- read_tsv(
				removeFileProtocol(file.path(
					...
					paste(
						organism,
						targetIdType,
						"phosphositeSeq.table",
						sep = "_"
					)
				)),
				col_names = c("phosphositeSeq", targetIdType),
				col_types = "cc",
				quote = ""
			)
			mappedInputGene <- inner_join(
				sourceMap,
				targetMap,
				by = c("phosphositeSeq")
			)
		...
		response <- POST(
			file.path(hostName, "api", "idmapping"),
			encode = "json",
			body = list(
				organism = organism,
				sourceType = sourceIdType,
				targetType = targetIdType,
				ids = inputGeneL,
				standardId = "phosphositeSeq",
				version = WEBGESTALT_DATA_VERSION
			)
		)
		...
		mappedInputGene <- data.frame(
			matrix(
				unlist(lapply(replace_null(mappedIds), FUN = function(x) {
					x[names]
				})),
				nrow = length(mappedIds),
				byrow = TRUE
			),
			stringsAsFactors = FALSE
		)
	...
	if (
		grepl("Uniprot", sourceIdType, fixed = TRUE) ||
			grepl("Ensembl", sourceIdType, fixed = TRUE) ||
			grepl("Refseq", sourceIdType, fixed = TRUE) ||
			grepl("GeneSymbol", sourceIdType, fixed = TRUE)
	) {
		##if the sourceIdType is Uniprot, Ensembl or Refseq, directly extract the gene level id####
		mappedInputGene$gene <- unlist(lapply(
			strsplit(mappedInputGene$userId, "_"),
			.combineG
		))
	} else {
		...
		if (targetIdType == "phosphositeUniprot") {
			mappedInputGene$gene <- unlist(lapply(
				strsplit(mappedInputGene[, targetIdType], "_"),
				.combineG
			))
		} else {
			if (startsWith(hostName, "file://")) {
				uniMapRes <- read_tsv(
					...
					col_names = c("phosphositeSeq", "phosphositeUniprot"),
					col_types = "cc",
					quote = ""
				) %>%
					filter(
						.data$phosphositeSeq %in% mappedInputGene$phosphositeSeq
					)
			} else {
				response <- POST(
					file.path(hostName, "api", "idmapping"),
					...
					body = list(
						organism = organism,
						sourceType = "phosphositeSeq",
						standardId = "phosphositeSeq",
						targetType = "phosphositeUniprot",
						ids = inputGeneL,
						version = WEBGESTALT_DATA_VERSION
					)
				)
				...
				if (length(uniMapRes$mapped) == 0) {
					return(idMappingError("empty"))
				}

				names <- c("sourceId", "targetId")
				uniMapRes <- data.frame(
					matrix(
						unlist(lapply(
							replace_null(uniMapRes$mapped),
							FUN = function(x) {
								x[names]
							}
						)),
						nrow = length(uniMapRes$mapped),
						byrow = TRUE
					),
					stringsAsFactors = FALSE
				)
			...
			uniMapRes$gene <- unlist(lapply(
				strsplit(uniMapRes[, "phosphositeUniprot"], "_"),
				.combineG
			))
			# Map ID may change nrow due to unmapped ones
			mappedInputGene <- uniMapRes %>%
				select(.data$phosphositeSeq, .data$gene) %>%
				right_join(mappedInputGene, by = "phosphositeSeq")
		}
	}
	#####Hard code#######
	if (
		grepl("Uniprot", sourceIdType, fixed = TRUE) ||
			sourceIdType == "phosphositeSeq"
	) {
	...
	if (grepl("GeneSymbol", sourceIdType, fixed = TRUE)) {
		geneType <- "genesymbol"
		outLink <- "https://www.ncbi.nlm.nih.gov/protein/"
	}
	...
	entrezgeneMapRes <- idMappingGene(
		organism = organism,
		dataType = "list",
		inputGene = mappedInputGene$gene,
		sourceIdType = geneType,
		targetIdType = "entrezgene",
		mappingOutput = FALSE,
		hostName = hostName
	)

	mergedRes <- entrezgeneMapRes$mapped %>%
		select(gene = .data$userId, .data$geneSymbol, .data$geneName) %>%
	...
	if (dataType == "rnk") {
		inputGene <- mergedRes %>%
			left_join(inputGene, by = c("userId" = sourceIdType)) %>%
			select(
				.data$userId,
				.data$geneSymbol,
				.data$geneName,
